...
typedef PlusPlayerHandle (*PlusPlayerCreatePlayer)(plusplayer::PlayerType type);
...
typedef std::vector<plusplayer::Track> (*PlusPlayerGetTrackInfo)(
    PlusPlayerHandle player);
typedef void (*PlusPlayerSetStreamingProperty)(PlusPlayerHandle player,
                                               const std::string& type,
                                               const std::string& value);
typedef bool (*PlusPlayerActivate)(PlusPlayerHandle player,
                                   const plusplayer::TrackType type);
typedef bool (*PlusPlayerDeactivate)(PlusPlayerHandle player,
                                     const plusplayer::TrackType type);
typedef bool (*PlusPlayerSetVolume)(PlusPlayerHandle player, int volume);
typedef bool (*PlusPlayerGetMemento)(PlusPlayerHandle player,
                                     plusplayer::PlayerMemento* memento);
typedef std::string (*PlusPlayerGetStreamingProperty)(PlusPlayerHandle player,
                                                      const std::string& type);
typedef int (*PlusPlayerGetTrackCount)(PlusPlayerHandle player,
                                       plusplayer::TrackType type);
typedef bool (*PlusPlayerSelectTrack)(PlusPlayerHandle player,
...
PlusPlayerProxy::PlusPlayerProxy() {
  ...
  if (app_res_path != nullptr) {
    ...
    if (version == "6.0") {
      lib_path += "/video_player_avplay/6.0/libplusplayer-wrapper.so";
    } else if (version == "6.5") {
      lib_path += "/video_player_avplay/6.5/libplusplayer-wrapper.so";
    } else if (version == "7.0") {
      lib_path += "/video_player_avplay/7.0/libplusplayer-wrapper.so";
    }
...
PlusPlayerHandle PlusPlayerProxy::CreatePlayer(plusplayer::PlayerType type) {
  ...
  *reinterpret_cast<void**>(&method_create_player) = Dlsym("CreatePlayer");
  if (method_create_player) {
    return method_create_player(type);
  }
  return nullptr;
}
...
void PlusPlayerProxy::SetStreamingProperty(PlusPlayerHandle player,
                                           const std::string& type,
                                           const std::string& value) {
  PlusPlayerSetStreamingProperty method_set_streaming_property;
  *reinterpret_cast<void**>(&method_set_streaming_property) =
      Dlsym("SetStreamingProperty");
  if (method_set_streaming_property) {
    return method_set_streaming_property(player, type, value);
  }
}

bool PlusPlayerProxy::Activate(PlusPlayerHandle player,
                               const plusplayer::TrackType type) {
  PlusPlayerActivate method_active;
  *reinterpret_cast<void**>(&method_active) = Dlsym("Activate");
  if (method_active) {
    return method_active(player, type);
  }
  return false;
}

bool PlusPlayerProxy::Deactivate(PlusPlayerHandle player,
                                 const plusplayer::TrackType type) {
  PlusPlayerDeactivate method_deactive;
  *reinterpret_cast<void**>(&method_deactive) = Dlsym("Deactivate");
  if (method_deactive) {
    return method_deactive(player, type);
  }
  return false;
}

bool PlusPlayerProxy::SetVolume(PlusPlayerHandle player, int volume) {
  PlusPlayerSetVolume method_set_volume;
  *reinterpret_cast<void**>(&method_set_volume) = Dlsym("SetVolume");
  if (method_set_volume) {
    return method_set_volume(player, volume);
  }
}

bool PlusPlayerProxy::GetMemento(PlusPlayerHandle player,
                                 plusplayer::PlayerMemento* memento) {
  PlusPlayerGetMemento method_get_memento;
  *reinterpret_cast<void**>(&method_get_memento) = Dlsym("GetMemento");
  if (method_get_memento) {
    return method_get_memento(player, memento);
  }
}

std::string PlusPlayerProxy::GetStreamingProperty(PlusPlayerHandle player,
                                                  const std::string& type) {
  PlusPlayerGetStreamingProperty method_get_streaming_property;
  *reinterpret_cast<void**>(&method_get_streaming_property) =
      Dlsym("GetStreamingProperty");
  if (method_get_streaming_property) {
    return method_get_streaming_property(player, type);
  }
}

int PlusPlayerProxy::GetTrackCount(PlusPlayerHandle player,
                                   plusplayer::TrackType type) {
  PlusPlayerGetTrackCount method_get_track_count;
  *reinterpret_cast<void**>(&method_get_track_count) = Dlsym("GetTrackCount");
  if (method_get_track_count) {
    return method_get_track_count(player, type);
  }
}

bool PlusPlayerProxy::SelectTrack(PlusPlayerHandle player,
                                  plusplayer::TrackType type, int index) {
  PlusPlayerSelectTrack method_select_track;
  *reinterpret_cast<void**>(&method_select_track) = Dlsym("SelectTrack");
  if (method_select_track) {
    return method_select_track(player, type, index);
  }
}